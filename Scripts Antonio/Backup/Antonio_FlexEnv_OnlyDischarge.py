...
#          self.observation_space = spaces.Box(-high, high, dtype=np.float32)
        ...
        self.charge_steps=np.linspace(-charge_lim,0,int((self.charge_lim/self.minimum_charge_step)+1)) #definition of actions
        # self.charge_steps = np.append(self.charge_steps,[0,0,0]) # For some reason, when there is only one possible solution of 0 it barely never gets chosen, compared to the others
    ...
    def step(self, action):
        ...
        # Modification António - Multiply by 100 to see better results as self.g is too small
        self.soc+=self.eta*self.g*100*self.dh
    ...
    def get_reward(self,action):
        ...
        if abs(action_)>self.l:
            reward1 = -50*((abs(action_)-self.l))
        else:
            reward1=0
        
        # A way to prevent the discharge to be smaller than the load at each instant     
        if self.l>0 and abs(action_)<self.l:
            reward2 = -50*((self.l-abs(action_)))
        else:
            reward2 = 0

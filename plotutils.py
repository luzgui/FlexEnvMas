...
def makeplot(T,delta,sol,gen,load,tar,env,var_1,var_2):
    ...
    ax.fill_between(t,sol.values.astype('float64'),facecolor="none",hatch="////", edgecolor='#1f77b4')
    ...
    ax.fill_between(t,useful_pv[0:T],label='self-consumption',color='#FFB300')
...
def makeplot(T,delta,sol,gen,load,tar,env,var_1,var_2):
    ...
    plot1, = ax1.plot(t,sol.values.astype('float64')+load[0:T].values.astype('float64'),label='Total load',color='#1f77b4')

    plot3, = ax1.plot(t,gen[0:T].values.astype('float64'),label='PV generation',color='#FF8b00')
    
    plot4 = ax1.fill_between(t,useful_pv[0:T].values.astype('float64'),interpolate=False,label='Useful PV',color='#FFB300')
    ...
    plot5 = ax1.fill_between(t,surplus_pv[0:T].values.astype('float64'),label='PV surplus',facecolor="none",hatch='..',edgecolor='#FFB300')
    
    
    
    plot2 = ax1.fill_between(t,sol.values.astype('float64'),facecolor="none",hatch="////", edgecolor='k',alpha=0.4) # , linewidth=0.0
    ...
    ax1.minorticks_on()
    ax1.grid(visible=True, which='minor', color='#999999', linestyle='-', alpha=0.07)
    
    ax2.grid(visible=True, which='major',alpha=0.07)
    ax2.minorticks_on()
    ax2.grid(visible=True, which='minor', color='#999999', linestyle='-', alpha=0.07)
